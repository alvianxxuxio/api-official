...
module.exports = {
  ...
  litterbox: async (content, time = "24h") => {
    try {
      const { ext } = (await fromBuffer(content)) || {};
      const formData = new FormData();
      formData.append("reqtype", "fileupload");
      formData.append("time", time);
      formData.append("fileToUpload", content, `${randomBytes}.${ext}`);
      ...
      if (!res.ok) throw new Error(`Gagal mengunggah ke Litterbox: ${res.statusText}`);

      return await res.text();
    } catch (error) {
      console.error("Error saat upload ke Litterbox:", error);
      throw new Error("Tidak dapat mengunggah file ke Litterbox. Silakan coba lagi.");
    }
  },
  skizo: async (buffer) => {
    try {
